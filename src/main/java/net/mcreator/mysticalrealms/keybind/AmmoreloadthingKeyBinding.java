...
package net.mcreator.mysticalrealms.keybind;
...
public class AmmoreloadthingKeyBinding extends MysticalrealmsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public AmmoreloadthingKeyBinding(MysticalrealmsModElements instance) {
		super(instance, 249);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.ammoreloadthing", GLFW.GLFW_KEY_R, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MysticalrealmsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
